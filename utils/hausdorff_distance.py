...
class HausdorffDistance(Metric):
    ...
    def __init__(self, distance_metric="euclidean", percentile=None, directed=False, crop=True):
        ...
        if isinstance(distance_metric, str) and (distance_metric in string_list):
...
def get_HausdorffDistance(pred, label, distance_metric='euclidean',  percentile=95, directed=False):
    metric = HausdorffDistance(distance_metric=distance_metric, percentile=percentile, directed=directed)
    if len(pred.size()) == 4:
        HausdorffDistanc = 0
        batch = pred.size(0)
        ...
        HausdorffDistanc = metric.eval()
        return HausdorffDistanc

if __name__ ==  '__main__':
    import torch
    x = torch.randn(512, 512) > 0.5
    y = torch.randn(512, 512) > -0.5
    HausdorffDistanc = get_HausdorffDistance(x, y)
    print(HausdorffDistanc)