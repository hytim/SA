...
class MeanSurfaceDistance(Metric):
    ...
    def __init__(self, symmetric=False, distance_metric="euclidean"):
        ...
        if isinstance(distance_metric, str) and (distance_metric in self.distance_metric_list):
            self.distance_metric = distance_metric
        else:
            print('distance_metrice error')
    ...
    def update(self, *inputs):
        ...
        self._y_pred_edges = morphology.binary_erosion(y_pred) ^ y_pred # ^: Xor. First erosion, then compute xor to get edge
...
def get_MSD(pred, label, symmetric=False, distance_metric='euclidean'):
    metric = MeanSurfaceDistance(symmetric=symmetric, distance_metric=distance_metric)
    if len(pred.size()) == 4:
        mean_average_distance = 0
        batch = pred.size(0)
        for i in range(batch):
            metric.clear()
            metric.update(pred[i,:], label[i,:], 1)
            t = metric.eval()
            if t == np.inf:
                batch = batch - 1
                continue
            mean_average_distance += t
        if batch != 0:
            return mean_average_distance / batch
        else:
            # return np.inf
            return 0
    else:
        # print('in else')
        metric.clear()
        metric.update(pred, label, 1)
        mean_average_distance = metric.eval()
        return mean_average_distance

if __name__ ==  '__main__':
    import torch
    x = torch.randn(6, 6) > 0.5
    y = torch.randn(6, 6) > 0.5
    mean_average_distance = get_MSD(x, y)
    print(mean_average_distance)