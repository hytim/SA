...
from model.layers.modules import conv_block, UpCat, UpCatconv, UnetDsv3, UnetGridGatingSignal3
from model.layers.grid_attention_layer import GridAttentionBlock2D, MultiAttentionBlock
from model.layers.channel_attention_layer import SE_Conv_Block
from model.layers.scale_attention_layer import scale_atten_convblock
from model.layers.nonlocal_layer import NONLocalBlock2D


class CA_Net(nn.Module):
    def __init__(self, feature_scale=4, is_deconv=True, is_batchnorm=True, out_size=1, 
                 nonlocal_mode='concatenation', attention_dsample=(1, 1)):
        super().__init__()
        self.is_deconv = is_deconv
        # self.in_channels = feature_ch
        ...
        # deep supervision
        self.dsv4 = UnetDsv3(in_size=filters[3], out_size=4, scale_factor=8)
        self.dsv3 = UnetDsv3(in_size=filters[2], out_size=4, scale_factor=4)
        self.dsv2 = UnetDsv3(in_size=filters[1], out_size=4, scale_factor=2)
...
if __name__ == '__main__':
    ...
    model = CA_Net()
    # model = U_Net(feature_ch=32, dilation=False, res=False, use_sa=False, use_ca=False, dsv=False, use_edge=True)
    # model = ATT_U(feature_ch=32)
    model = model.cuda()
    out = model(torch.randn(2,1,256,256).cuda())
    summary(model, [(1, 256, 256)])
    # block = conv_block(16, 32, False)
