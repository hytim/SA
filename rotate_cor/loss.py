...
CLASS_NUM = [782, 4694, 349, 231, 759, 85, 110]
CLASS_WEIGHT = torch.Tensor([7010/i for i in CLASS_NUM]).cuda()
...
class LabelSmoothingCrossEntropy(object):
    def __init__(self, epsilon=0.1, reduction='mean'):
        ...
        self.base_loss = torch.nn.CrossEntropyLoss(
            reduction='mean', weight=CLASS_WEIGHT)
...
class smoothcrossentropy(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        for i in range(label.size(0)):
            index = (label[i,:]==1).nonzero(as_tuple=True)[0].item()
            label[index-2, index+2] = self.weight
...
def cam_activation(batch_feature, channel_weight):
    ...
    activations = torch.reshape(
        batch_feature, (batch_feature.shape[0], -1, batch_feature.shape[3]))
...
if __name__ == '__main__':
    ...
    # predict = torch.randint(4, (8,4))
    predict = torch.randn(8, 4)
    gt = torch.randint(4, (8,1))
    # one_hot_predict = one_hot_encoder(predict, 4)
    one_hot_gt = one_hot_encoder(gt, 4)
    l = loss(predict, gt)
    pass
